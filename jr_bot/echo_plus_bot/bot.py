...
async def cmd_start(message: types.Message):
    await message.answer("Assalomu alekum!. Men echo botman",protect_content=True )
...
async def cmd_weather(message: types.Message):
    await message.reply("🌤 Bugun ob-havo yaxshi!",protect_content=True)
...
async def salom_txt(message: types.Message):
    await message.reply("✋🏻Vaalaykum assalom! Qalaysiz?",protect_content=True)
...
async def raxmat_txt(message: types.Message):
    await message.reply("Arzimaydi🤝",protect_content=True)
